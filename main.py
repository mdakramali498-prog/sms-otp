...
def check_number_via_bot(number: str) -> str:
    ...
    async def _check():
        client = TelegramClient(os.path.join(BASE_DIR, 'checker_session'), API_ID, API_HASH)
    ...
    while True:
        ...
        try:
            result = api.order_number(
                ...
                country=s["country"],
                max_price=api_max or max_price,
...
def _do_bulk_order(uid: int, s: dict, msg_id: int = None):
    ...
    bulk_check_report = check_multiple_numbers_via_bot(bought_numbers) if bought_numbers else ""
...
def _cancel_order(uid: int, msg: types.Message, order_id: str):
    ...
    # Check if message text itself signals a successful cancellation
